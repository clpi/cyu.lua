local hsluv = require("cayu.hsluv")
...
-- local ns = vim.api.nvim_create_namespace("cayu")
...
function util.debug(colors)
  colors = colors or require("cayu.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "cayu" then
    vim.cmd([[autocmd! cayu]])
    vim.cmd([[augroup! cayu]])
  end
end
...
function util.autocmds(config)
  vim.cmd([[augroup cayu]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("cayu.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/cayu/** nested colorscheme cayu]])
  end
